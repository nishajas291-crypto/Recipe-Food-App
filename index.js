...
favouritesButton.classList.add("favourites-btn");
document.querySelector("header")?.appendChild(favouritesButton);
...
},
{
  ...
  strCategory: "Healthy Drink",
  strInstructions: " Blend blueberries, banana, yogurt, and honey until smooth and creamy.Serve chilled with a few fresh blueberries on top for a refreshing taste.",
  strYoutube: "https://www.youtube.com/watch?v=d5x5xLknyrY",  // example video link
...
},
{
  ...
  strCategory: "Healthy Drink",
  strInstructions: "Blend watermelon cubes, mint leaves, lemon juice, and ice Strain if needed for a smoother texture.Add a pinch of salt or honey for taste Serve immediately, chilled and fresh",
  strYoutube: "https://www.youtube.com/watch?v=d5x5xLknyrY",  // example video link
...
const sweets = [ {
        ...
        ingredients: ["1/2 cup Gram Flour", "Ghee", "1/4Sugar", " Cardamom"],
        nutrients: {
    calories: "330 kcal" ,
    carbs: "20g",
    protein: "5g",
    ...
    },
    {
        ...
        ingredients: ["1/4Chia Seeds", " 1/2Milk", "Mango Puree", "Honey"],
        nutrients: {
    calories: "340 kcal" ,
    carbs: "25g",
    protein: "5g",
    ...
    },
    {
        ...
        ingredients: ["Apples", "Cinnamon", "Oats", "Butter", "1/2kg Sugar"],
        nutrients: {
    calories: "230 kcal" ,
    carbs: "20g",
    protein: "5g",
    ...
    },
    {
        ...
        ingredients: [" 1/2 cup Semolina (Rava)", "Ghee", " 1/2 cup Sugar", "  1 Water", "Cardamom"],
        nutrients: {
    calories: "170 kcal" ,
    carbs: "20g",
    protein: "5g",
    ...
    },
    {
        ...
        ingredients: ["Khoya (Mawa)", " 1kg All-purpose flour", " 1/2  cup Sugar", "Water", "Cardamom", "Ghee"],
        nutrients: {
   calories: "140 kcal" ,
    carbs: "25g",
    protein: "5g",
    ...
    },
    {
        ...
        ingredients: ["Chocolate", "Butter", "1/4 cup Sugar", "Eggs", " 1/2 cup Flour", "Vanilla Extract"],
        nutrients: {
    calories: "180 kcal" ,
    carbs: "45g",
    protein: "5g",
    ...
    {
        idMeal: "SW007",
        strMeal: "Bluberry Lavender Tartlet",
        strMealThumb: "https://i.pinimg.com/736x/4b/db/15/4bdb15ae73cd5d099a0aa5d18876e073.jpg",
        strCategory: "Sweet",
        strInstructions: "Mix chocolate, butter, sugar, eggs, and flour. Bake until set and serve.",
        strYoutube: "https://www.youtube.com/watch?v=kpD9dVvR7bY",
    ...
    },
    {
        idMeal: "SW008",
        strMeal: "Milk Barf",
        strMealThumb: "https://i.pinimg.com/1200x/ac/96/c2/ac96c2dfb6a00807fa48629951921d5d.jpg",
        strCategory: "Sweet",
        strInstructions: "Mix chocolate, butter, sugar, eggs, and flour. Bake until set and serve.",
        strYoutube: "https://www.youtube.com/watch?v=1bC5Spv4pC0",
...
function displayMeals(meals, isCustom = false) {
  ...
  if (meals && meals.length) {
    meals.forEach((meal) => {
      ...
      mealItem.innerHTML = `
        ...
        <h3>${meal.strMeal}</h3>
        <button class="fav-btn" data-id="${meal.idMeal}">❤️ Add to Favourites</button>
      `;
  ...
  // 🩷 Add favourite button events
  document.querySelectorAll('.fav-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const mealId = e.target.dataset.id;
      const meal = isCustom
        ? [...healthyDrinks, ...sweets].find(m => m.idMeal === mealId)
        : meals.find(m => m.idMeal === mealId);
      toggleFavourite(meal);
    });
  });
}

// 🧠 FAVOURITES LOGIC (using LocalStorage)
...
function toggleFavourite(meal) {
  ...
  const exists = favourites.find((m) => m.idMeal === meal.idMeal);

  if (exists) {
    favourites = favourites.filter((m) => m.idMeal !== meal.idMeal);
    alert(`${meal.strMeal} removed from favourites 💔`);
  } else {
    favourites.push(meal);
    alert(`${meal.strMeal} added to favourites ❤️`);
  }
...
function showFavourites() {
  ...
  if (favourites.length === 0) {
    mealList.innerHTML = '<p>No favourites yet! ❤️</p>';
  } else {
    displayMeals(favourites, true);
  }
}
...
async function handleMealClick(e) {
  ...
  let meal;
  if (isCustom) {
    meal = [...healthyDrinks, ...sweets].find((m) => m.idMeal === mealId);
...
function showMealDetailsPopup(meal, isCustom = false) {
  ...
  if (isCustom && meal.nutrients) {
    nutrientsHTML = `
      ...
      <ul>${Object.entries(meal.nutrients)
